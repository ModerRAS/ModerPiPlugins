...
部门主管不是“任务复杂时才加的装饰层”。它的主要意义是替 Boss 持续盯员工，使 Boss 不必消费所有员工的过程消息。Boss 可以直接安排少量临时工作，但常规并行执行应经过主管。
...
UI 固定采用分屏思路，聊天和执行细节在协议层分流：

```text
+--------------------------------+--------------------------+
| 用户与 focused Boss 的主聊天   | 部门 / Worker 活动区     |
|                                | 可切换角色与 Inspector   |
+--------------------------------+--------------------------+
| 状态栏与主输入框（始终可输入）                            |
+-----------------------------------------------------------+
...
用户始终有权看到所有角色的正常文本；右侧区域承载主管、Worker 和非 focused Boss 的群聊活动，选中角色后可展开其实现细节。这样用户看得到全部，但员工消息不会冲散用户与当前 Boss 的聊天。

### 主聊天

左侧主聊天显示用户与 focused Boss 的连续对话。其他 Boss、主管和 Worker 的正常文本在右侧群聊活动区实时显示，并保存在同一个正式大群日志中。用户可以切换视图或角色：
...
这些视图只影响 UI，不改变投递、权限或 Agent 上下文。
...
每个主管至少应允许 4 个 Worker，避免重新形成单员工串行瓶颈。Agent 数量与真正并发的模型请求数可以分别配置，但首版不为供应商限流增加复杂调度；遇到实际 429 或本机资源问题后再处理。
...
用户与 focused Boss 的聊天固定在主区域，其他角色的正常文本和 Inspector 放在旁侧区域。信息分离方案已经确定；剩余工作是验证 Pi overlay/自定义组件能否在高频更新、窄终端和持续输入时稳定实现，必要时再调整为 drawer。
...
### 11. Inspector 使用实验性 TUI 能力

先实现并测试非抢焦点 overlay 分屏。需要验证高频刷新、窄终端、Esc、主 transcript 同时更新和大量工具输出时的性能；只有实际不稳定时才降级为 drawer。

### 12. 组织层级会产生协调成本

三层结构能降低 Boss 的监督负担，但每次派工、汇总和转述都会消耗 token 和时间。主管必须能够直接处理部门日常事件，并只上报有价值的信息，否则只是增加一次 LLM 转发。
...
- Boss 默认只收到主管汇报，但可主动下钻。
- Worker 默认只收到 `@自己` 和必要任务内容；主管看到整个部门。
